...
# --- Load limma results -------------------------------------------------------
results_list <- list(
  Baseline_CREvPLA  = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 1),
  Training_CREvPLA  = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 2),
  Training_CRE      = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 3),
  Training_PLA      = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 4),
  Interaction_supp  = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 5),
  Baseline_SURVvCTL = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 6),
  Training_SURVvCTL = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 7),
  Training_SURV     = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 8),
  Baseline_FvM      = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 9),
  Training_FvM      = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 10),
  Training_F        = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 11),
  Training_M        = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 12),
  Interaction_sex   = read_excel(file.path(report_dir, "limma_results_all_plasma.xlsx"), sheet = 13)
)
...
# =============================================================================
# ADD ENSEMBL IDs TO ALL DATASETS (UniProt → Ensembl)
# =============================================================================

add_ensembl <- function(df) {
  id_map <- bitr(
    df$uniprot,
    fromType = "UNIPROT",
    toType   = "ENSEMBL",
  ...
  id_map <- id_map[!duplicated(id_map$UNIPROT), ]
  
  merged <- merge(df, id_map, by.x = "uniprot", by.y = "UNIPROT", all.x = FALSE)
  merged <- merged[!is.na(merged$ENSEMBL), ]
  merged <- merged[!duplicated(merged$ENSEMBL), ]
  colnames(merged)[colnames(merged) == "ENSEMBL"] <- "Ensembl"
  
  cat(sprintf("Mapped %d / %d proteins to Ensembl (%.1f%%)\n",
              nrow(merged), nrow(df), 100 * nrow(merged) / nrow(df)))
  merged
}

# Apply to all datasets
results_list <- lapply(results_list, add_ensembl)

cat("Ensembl IDs added to all datasets.\n")
...
# --- Plot ORA ---
plot_ora("Training_CRE",      "CREvPLA",  ont = "BP", type = "dot")
...
# --- Compare to HPA: Colorectal Cancer v Healthy --------

input_dir <- file.path(base_dir, "01_input")

# ── 1. Load HPA data ──────────────────────────────────────────────────────────
hpa <- read_csv(file.path(input_dir, "HPA_CRCvHEALTHY_plasma.csv")) %>%
  dplyr::rename(
    gene       = Protein,
...
# ── 2. Pick which sheet to compare (e.g. Baseline CRE vs PLA) ────────────────
my_results <- results_list$Baseline_SURVvCTL %>%   # ← change as needed
  dplyr::select(gene, logFC, adj.P.Val) %>%
  dplyr::rename(logFC_limma = logFC, padj_limma = adj.P.Val)

# ── 3. Inner join on gene symbol ──────────────────────────────────────────────
compared <- inner_join(hpa, my_results, by = "gene")

cat(sprintf("Proteins in HPA:        %d\n", nrow(hpa)))
cat(sprintf("Proteins in my results: %d\n", nrow(my_results)))
...
print(compared %>% dplyr::select(gene, logFC_HPA, logFC_limma, agreement))

# ── 5. Scatter plot: logFC correlation ───────────────────────────────────────
ggplot(compared, aes(x = logFC_HPA, y = logFC_limma, color = agreement)) +
  ...
  labs(
    ...
    y     = "SURV v CTL logFC",
    title = "logFC Comparison: HPA vs Our Results",
    color = "Direction"
  ) +
  theme_bw(base_size = 12) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

# ── 6. Proteins in HPA but NOT in your data ───────────────────────────────────
not_found <- anti_join(hpa, my_results, by = "gene")
cat("\nHPA proteins not in your dataset:\n")
print(not_found$gene)
